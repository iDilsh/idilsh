...
import { Eye, EyeOff, Trash2, ChevronDown, ChevronRight, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="h-5 text-[10px] bg-zinc-800 border-zinc-700 text-zinc-300 flex-1 min-w-0"
      />
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Slider
        ...
        className="flex-1"
      />
      <span className="text-[10px] text-zinc-400 w-10 text-right">{value}{suffix}</span>
    </div>
  );
}
...
export default function LayerEffectsPanel() {
  ...
  const hasEnabledEffects = Object.entries(effects).some(
  ...
  const updateEffect = (type: EffectType, value: unknown) => {
    updateLayerEffects(activeLayer.id, { [type]: value });
  };

  const toggleEnabled = (type: EffectType) => {
    const current = effects[type] as { enabled: boolean } | undefined;
    if (current) {
      updateEffect(type, { ...current, enabled: !current.enabled });
    } else {
      // Add with defaults, enabled
  ...
  const removeEffect = (type: EffectType) => {
    const current = effects[type] as { enabled: boolean } | undefined;
    if (current) {
      updateEffect(type, { ...current, enabled: false });
    }
  };
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="h-5 text-[10px] px-2 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700"
            >
              + Add Effect
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="bg-zinc-800 border-zinc-700 min-w-40">
            {effectOrder.map((type) => (
              <DropdownMenuItem
                key={type}
                onClick={() => addNewEffect(type)}
                className="text-[11px] text-zinc-300 focus:bg-zinc-700 focus:text-zinc-100"
              >
                {effectLabels[type]}
              </DropdownMenuItem>
            ))}
            ...
            <div key={type} className="border-b border-zinc-800">
              ...
              <div className="flex items-center gap-1 px-2 py-1 hover:bg-zinc-800">
                ...
                >
                  ...
                  <span className={`text-[11px] ${effect.enabled ? 'text-zinc-300' : 'text-zinc-600'}`}>
                    {effectLabels[type]}
                  </span>
                </button>
                <button
                  className="text-zinc-600 hover:text-red-400 flex-shrink-0"
                  onClick={() => removeEffect(type)}
                >
                  <Trash2 size={10} />
                </button>
              </div>
              ...
              {isExpanded && (
                <div className="bg-zinc-850">
                  {type === 'dropShadow' && effects.dropShadow && (
                    <ShadowEffectEditor
                      effect={effects.dropShadow}
                      onChange={(e) => updateEffect('dropShadow', e)}
                    />
                  )}
                  {type === 'innerShadow' && effects.innerShadow && (
                    <ShadowEffectEditor
                      effect={effects.innerShadow}
                      onChange={(e) => updateEffect('innerShadow', e)}
                    />
                  )}
                  {type === 'stroke' && effects.stroke && (
                    <StrokeEffectEditor
                      effect={effects.stroke}
                      onChange={(e) => updateEffect('stroke', e)}
                    />
                  )}
                  {type === 'outerGlow' && effects.outerGlow && (
                    <GlowEffectEditor
                      effect={effects.outerGlow}
                      onChange={(e) => updateEffect('outerGlow', e)}
                    />
                  )}
                  {type === 'innerGlow' && effects.innerGlow && (
                    <GlowEffectEditor
                      effect={effects.innerGlow}
                      onChange={(e) => updateEffect('innerGlow', e)}
                    />
                  )}
                  {type === 'bevelEmboss' && effects.bevelEmboss && (
                    <BevelEffectEditor
                      effect={effects.bevelEmboss}
                      onChange={(e) => updateEffect('bevelEmboss', e)}
                    />
                  )}
                  {type === 'satin' && effects.satin && (
                    <SatinEffectEditor
                      effect={effects.satin}
                      onChange={(e) => updateEffect('satin', e)}
                    />
                  )}
                </div>
              )}
